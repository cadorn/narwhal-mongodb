...
Collection.prototype = {
    ...
    findOne: function(obj) {
        ...
        if (obj && typeof(obj) != "object")
            dbo = { "_id": Util.createObjectId(obj) };
    ...
    insert: function(docs) {
        var insertable = UTIL.isArrayLike(docs) ?
                         UTIL.map(docs, Util.createBDObject) : Util.createBDObject(docs);

        this.collection.insert(insertable);

        // put _id(s) onto docs
        if(UTIL.isArrayLike(docs)) {
            for( var i=0 ; i<docs.length ; i++ ) {
                docs[i]._id = insertable[i].get("_id").toString();
            }
        } else {
            docs._id = insertable.get("_id").toString();
        }
    },
    update: function(criteria, obj) {
        
        if (criteria && typeof(criteria) != "object") {
            criteria = { "_id": Util.createObjectId(criteria) };
        }

        var dbo = Util.createBDObject();
        UTIL.forEach(obj, function(item) {
            dbo.put(item[0], Util.createBDObject(item[1]));
        });

        this.collection.update(Util.createBDObject(criteria), dbo);
    },
